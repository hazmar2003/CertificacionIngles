...
export default function TeacherForm({ teacher: initialTeacher, onSubmit, onCancel }: TeacherFormProps) {
  ...
  const validateEmail = (email: string) => {
    ...
    if (!/^[^@]+@profesor\.uci\.cu$/.test(email)) {
      return "El email debe tener el formato username@profesor.uci.cu";
    }
    return "";
  };

  const validatePassword = (password: string) => {
    if (!initialTeacher.id && !password) return "La contraseña es obligatoria";
  ...
  const validatePasswordConfirm = (confirm: string) => {
    if (teacher.user?.password && teacher.user.password !== confirm) {
      return "Las contraseñas no coinciden";
    }
    if (!initialTeacher.id && !confirm) return "Debe confirmar la contraseña";
    return "";
  };
  ...
  const validateForm = () => {
    ...
    if (!teacher.user?.firstName) newErrors.firstName = "El nombre es obligatorio";
    if (!teacher.user?.lastName) newErrors.lastName = "El apellido es obligatorio";
    if (!teacher.user?.officialId) newErrors.officialId = "El carnet es obligatorio";
    if (!teacher.user?.email) newErrors.email = "El email es obligatorio";

    // Validar campos con reglas específicas
    newErrors.officialId = validateOfficialId(teacher.user?.officialId || '');
    newErrors.email = validateEmail(teacher.user?.email || '');
    newErrors.password = validatePassword(teacher.user?.password || '');
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {(!initialTeacher.id || teacher.user?.password) && (
          <>
            <div>
      ...
      <div className="flex justify-end space-x-3 pt-4 border-t">
        ...
        >
          {initialTeacher?.id ? 'Actualizar Profesor' : 'Guardar Profesor'}
        </button>
      </div>
    </form>
  );
